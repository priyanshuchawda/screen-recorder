...
constexpr RecordingProfile profile_for(bool high_quality, bool on_ac, int requested_fps) {
    const int fps = normalized_fps(requested_fps);
    if (high_quality) {
        return {1920, 1080, fps, fps == 60 ? 10'000 : 8'000, true, on_ac};
    }
    if (!on_ac) {
        return {848, 480, 15, 1'500, false, false};
    }
    return {848, 480, fps, fps == 60 ? 6'000 : 4'000, false, true};
}
