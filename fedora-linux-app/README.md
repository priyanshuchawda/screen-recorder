...
- Every recording has a neighboring `.diagnostics.txt` file with selected encoder, power state, profile, audio/camera choices, and completion status.
...
- Existing `.partial.mp4` files are detected at startup and preserved; no unfinished recording is silently overwritten or renamed as complete.
